...
import { platform, type AfurecoPendingTakeFile } from "../lib/platform";
...
export function AfurecoProjectScreen() {
  ...
  const [pendingFilesDirectory, setPendingFilesDirectory] = useState<string | null>(null);
  const [fileLocationMessage, setFileLocationMessage] = useState("");
  ...
  useEffect(() => {
    ...
    void getAfurecoProject(projectId).then((nextProject) => {
      if (!active) return;
      setPendingFilesDirectory(null);
      setFileLocationMessage("");
  ...
  const pendingTakes = useMemo(() => takes.filter((take) => take.reviewStatus === "SUBMITTED" && take.syncStatus !== "SYNCED"), [takes]);
  ...
  const localStoragePath = pendingFilesDirectory ?? `このPC / P.N.E.アプリ内データ / アフレコ / ${project?.projectName ?? "収録案件"}`;
  ...
  const startRecording = async () => {
    setError(""); setPendingRecording(null); setPendingFilesDirectory(null); setFileLocationMessage(""); setLevel(0);
    try {
  ...
  const savePendingTake = async () => {
    ...
    try {
      ...
      setPendingRecording(null); setPendingFilesDirectory(null); setFileLocationMessage(""); setMemo(""); await refreshTakes();
    } catch (cause) { setError(cause instanceof Error ? cause.message : "テイクを保存できませんでした。"); }
  };

  const revealPendingFiles = async () => {
    if (!pendingTakes.length) {
      setError("提出待ちの録音ファイルがありません。");
      return;
    }
    ...
    setFileLocationMessage("");
    try {
      const files = (await Promise.all(pendingTakes.map(async (take): Promise<AfurecoPendingTakeFile | null> => {
        const blob = await loadTakeAudio(take.fileId);
        return blob ? { takeId: take.takeId, blob } : null;
      }))).filter((file): file is AfurecoPendingTakeFile => file !== null);
      if (files.length !== pendingTakes.length) throw new Error("提出待ちの録音ファイルを読み込めませんでした。");
      const result = await platform.revealAfurecoPendingFiles(project.projectId, files);
      setPendingFilesDirectory(result.directory);
      setFileLocationMessage(result.native
        ? `${result.fileCount}件の提出待ちファイルをローカルフォルダへ書き出し、エクスプローラーで表示しました。`
        : `${result.fileCount}件の提出待ちファイルをダウンロードしました。`);
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "提出待ちファイルを表示できませんでした。");
    } finally {
  ...
  return <div className="page afureco-project-page">
    ...
    {allLinesSubmitted && !pendingRecording && <section className="afureco-complete-banner" role="status"><div className="afureco-complete-icon">✓</div><div className="afureco-complete-copy"><p className="eyebrow">RECORDING COMPLETE</p><h2>すべてのセリフパックの収録が終わりました！</h2><p>全{lines.length}セリフが提出待ちとして保存されています。</p><div className="afureco-save-location"><span>保存先</span><code>{localStoragePath}</code></div>{fileLocationMessage && <small className="afureco-file-location-message" role="status">{fileLocationMessage}</small>}<small>{pendingFilesDirectory ? "提出用に書き出したWAVファイルがこのフォルダにあります。" : "収録データはこのPCのP.N.E.アプリ内に保存されています。ボタンから提出用ファイルを表示できます。"}</small></div><button className="button secondary afureco-reveal-files" onClick={() => void revealPendingFiles()} disabled={exportingPendingFiles || !pendingTakes.length}>{exportingPendingFiles ? "準備中…" : "提出待ちファイルを表示"}</button></section>}
    ...
    <div className="afureco-layout">
      ...
      <section className="afureco-record-panel" aria-label="セリフ収録">
        ...
        <div className="record-save-location" aria-label="録音ファイルの保存先"><div><span>録音ファイルの保存先</span><b>{selectedTakes.length ? `${selectedTakes.length}テイク保存済み` : "提出待ちに保存すると保存されます"}</b></div><code>{localStoragePath}</code><small>{pendingFilesDirectory ? "提出用に書き出したWAVファイルの保存先です。" : "収録データはブラウザ保存領域（IndexedDB）に保存されています。提出用ファイルは完了時のボタンから書き出せます。"}</small></div>
