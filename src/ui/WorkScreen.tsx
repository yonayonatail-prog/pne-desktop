...
export function WorkScreen() {
  ...
  const nameCallAvailable = Boolean(work?.capabilities.name_call_supported && work.nameSlots.length);
  ...
  return <div className="page work-page">
    ...
    <section className="work-hero">
      ...
      <div className="work-copy"><StatusPill state={work.state} /><p className="eyebrow">POCKET NARRATIVE</p><h1>{work.title}</h1><p className="author">{work.author}</p><p className="description">{work.description}</p>
        ...
        <div className="capabilities"><span>✓ オフライン再生</span>{nameCallAvailable && <span>✓ 名前呼び</span>}<span>✓ Reaction分岐</span>{work.capabilities.mobile_transfer_supported && <span>✓ スマホ転送</span>}</div>
        <div className="hero-actions">{saved && saved.snapshot.status !== "ENDED" && <Link className="button primary large" to={`/play/${work.workId}/${work.version}?resume=1`} onClick={() => void unlockAudioPlayback().catch(() => {})}>続きから <span>→</span></Link>}{nameCallAvailable ? <Link className={saved ? "button secondary" : "button primary large"} to={`/works/${work.workId}/${work.version}/name`}>{saved ? "最初から準備" : "名前を設定する"} <span>→</span></Link> : <Link className={saved ? "button secondary" : "button primary large"} to={`/play/${work.workId}/${work.version}?fallback=1&restart=1`} onClick={() => { void unlockAudioPlayback().catch(() => {}); void platform.deleteSession(work.workId, work.version); }}>再生を始める <span>→</span></Link>}{nameCallAvailable && <Link className="button secondary" to={`/play/${work.workId}/${work.version}?fallback=1&restart=1`} onClick={() => { void unlockAudioPlayback().catch(() => {}); void platform.deleteSession(work.workId, work.version); }}>名前なしで始める</Link>}</div>
      </div>
    </section>
    <section className="preflight-panel"><h2>再生の準備</h2><div className="check-row"><span className="check-icon">✓</span><div><b>作品データ</b><small>検証済み・このPCに保存されています</small></div><strong>準備完了</strong></div>{nameCallAvailable && <div className="check-row warn"><span className="check-icon">◇</span><div><b>Irodori名前音声モデル</b><small>名前画面から固定リビジョンのモデル約1.3GBを取得し、このPCのWebGPUで生成します</small></div><strong>名前画面で確認</strong></div>}</section>
  </div>;
}
